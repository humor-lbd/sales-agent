/*
文件作用：
- 定义 sales-agent 的核心表结构、主键和索引。
- 阅读这个 SQL 文件时，建议先看表或数据分组，再看每条语句影响哪些对象。
*/

-- 建表语句：定义 sa_sales_region 的结构，方便后续查询和统计。
...
-- 建表语句：定义 sa_sales_rep 的结构，方便后续查询和统计。
...
-- 建表语句：定义 sa_product 的结构，方便后续查询和统计。
...
-- 建表语句：定义 sa_sales_order 的结构，方便后续查询和统计。
...
-- 建表语句：定义 sa_chat_memory 的结构，方便后续查询和统计。
